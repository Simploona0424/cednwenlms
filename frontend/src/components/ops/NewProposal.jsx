...
function NewProposal() {
  ...
  const handleFormSubmit = async (e, buttonName) => {

    e.preventDefault();
    try {
      let updatedFormData = { ...formData, buttonName };
      if (clientType === "New Client") {

        updatedFormData = { ...updatedFormData, ClientName: companyName ,fullName:fullName,emailId:emailId };

        const res = await axios.post("https://cedwenlms.onrender.com/api/cmdata", {
          companyName,
          clientID,
          fullName,
          emailId,
      ...
      await axios.post("https://cedwenlms.onrender.com/api/newproposal", payload);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("https://cedwenlms.onrender.com/api/userdata");
  ...
  const handleaddProductSubmit = async (e) => {
    ...
    try {
      await axios.post("https://cedwenlms.onrender.com/userdata", formdata);
      setAddProduct(false);
    } catch (error) {
  ...
  const fetchadminData = async () => {
    try {
      const adminDataRes = await axios.get(
        "https://cedwenlms.onrender.com/api/adminData"
      );
  ...
  const fetchdepartmentData = async () => {
    try {
      const response = await axios.get("https://cedwenlms.onrender.com/api/getdepartment");
    ...
    <div>
      ...
      <main className="ml-60 flex-1 p-5 ">
        ...
        <section className="border  rounded-xl mt-5">
          <form onSubmit={(e) => handleFormSubmit(e, buttonName)}>
            ...
            {clientType === "Existing Client" && (<div className="bg-[#E8E8E8] px-4 text-sm max-xl:text-xs font-normal py-8 ">
              <div>
                ...
                <select
                  className="border px-2  font-normal rounded-md ml-4 py-[0.12rem] w-[35%] h-8"
                  name="pets"
                  id="pet-select"
              ...
              <div className="overflow-x-auto">
                <table className="min-w-[100%] table-auto rounded-lg shadow-sm border border-separate border-spacing-0">
                  <thead className="bg-[#E84000] rounded-t-lg">
                    <tr>
                      <th className="px-2 w-10  whitespace-nowrap text-center py-[0.35rem] text-white text-sm max-xl:text-xs font-medium  border-b border-gray-300 rounded-tl-lg">
                        Sr No.
                      </th>
                      ...
                      <th className="px-2 w-32 py-[0.35rem] text-white text-sm max-xl:text-xs font-medium text-center border-b border-gray-300">
                        Rate
                      </th>
                      <th className="px-2 py-[0.35rem] text-white text-sm max-xl:text-xs font-medium text-center border-b border-gray-300">
                        Total
                      </th>
                      <th className="px-2 w-20 py-[0.35rem] text-white text-sm max-xl:text-xs font-medium text-center border-b border-gray-300">
                        CGST
                      </th>
                      <th className="px-2 w-20 py-[0.35rem] text-white text-sm max-xl:text-xs font-medium text-center border-b border-gray-300">
                        SGST
                      ...
                      <th className="px-2 w-10 py-[0.35rem] text-white text-sm max-xl:text-xs font-medium text-center border-b border-gray-300 rounded-tr-lg"></th>
                    </tr>
                  </thead>
                  ...
                  <tbody>
                    {rows.map((row, index) => (
                      ...
                      >
                        ...
                        <td className="px-2 text-center border-b border-gray-300 text-sm max-xl:text-xs">
            ...
            <section className="px-4 py-8 flex justify-between text-sm max-xl:text-xs font-normal">
              <div className="flex gap-3 items-end">
                <button
                  type="submit"
                  name="Save as Draft"
                  className="w-36 h-8 bg-[#E84000] text-white rounded-md"
                  onClick={() => setButtonName("Draft")}
                >
                  Save as Draft
                </button>
                <button
                  ...
                  className="w-40 h-8 text-[#E84000] border-[#E84000] border rounded-md"
                  onClick={() => setButtonName("Approval")}
                >
                  Submit for Approval
                </button>
                <button
              ...
              <div>
                ...
                {/* <div className="flex mt-1 gap-1 font-normal">
                  <p className="font-semibold">Total Amount in Words: </p>
                  <p>{numberToWords(Math.round(formData.Amount || 0))}</p>
                </div> */}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white border rounded-lg shadow-lg w-[75%] max-w-6xl">
              <div className="flex justify-between items-center p-5 border-b">
                <h1 className="font-semibold text-xl max-xl:text-base text-[#E84000]">
                  {editingIndex !== null
                    ? "Edit Client Master"
                    : "New Client Master"}
                </h1>
                <button
                  onClick={() => setNewCustomers(false)}
